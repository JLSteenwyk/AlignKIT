...
class PolicyNetwork(nn.Module):
    ...
    def __init__(
        ...
        # Shared trunk (everything except final projection)
        self.trunk = nn.Sequential(*layers)
        self.policy_head = nn.Linear(prev_dim, output_dim)
        self.value_head = nn.Linear(prev_dim, 1)

        # Backward-compatible sequential view (trunk + policy head)
        self.network = nn.Sequential(*layers, self.policy_head)
    ...
    def get_value(self, state: torch.Tensor) -> torch.Tensor:
        """Compute value estimate for a state (or batch of states).

        Args:
            state: Tensor of shape (input_dim,) or (batch, input_dim).

        Returns:
            Value scalar (shape () or (batch,)).
        """
    ...
    def evaluate_actions(
        ...
        """Evaluate log-probs, entropies, and values for a batch of state-action pairs.
        ...
        Returns:
            (log_probs, entropies, values) each of shape (batch,).
        """
