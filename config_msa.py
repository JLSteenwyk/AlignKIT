...
class MSAConfig:
    ...
    mafft_op_values: List[float] = field(
        default_factory=lambda: [0.5, 1.0, 1.53, 2.5, 4.0]
    )
    mafft_ep_values: List[float] = field(
        default_factory=lambda: [0.0, 0.1, 0.5, 1.0]
    )
    ...
    mafft_maxiterate_values: List[int] = field(
        default_factory=lambda: [0, 2, 10, 100]
    )
    ...
    muscle_perm_values: List[str] = field(
        default_factory=lambda: ["none", "abc", "acb", "bca"]
    )
    ...
    clustalo_iter_values: List[int] = field(
        default_factory=lambda: [0, 1, 2, 3, 5]
    )
    ...
    # --- Subprocess ---
    subprocess_timeout: Optional[int] = None  # seconds (None = no limit)
    mafft_threads: int = 4
    ...
    use_benchmarks: bool = True  # include OXBench, SABRE, HOMSTRAD
